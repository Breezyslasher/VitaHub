...
void SctpTransport::doRecv() {
	...
	--mPendingRecvCount;
	// Vita: this receive buffer must NOT live on the stack. pthread stacks are
	// heap-allocated on the Vita, so a 64KB stack frame here overruns the worker
	// thread's stack into adjacent heap chunks and corrupts the global heap
	// (observed: main thread aborting in free() right after the SCTP handshake).
	// A thread_local heap buffer is off-stack, allocated once per worker, and
	// reused (doRecv holds mRecvMutex, so no concurrent use within a thread).
	static constexpr size_t bufferSize = 65536;
	static thread_local std::vector<byte> recvBuffer(bufferSize);
	byte *buffer = recvBuffer.data();
	try {
