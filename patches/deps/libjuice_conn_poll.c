...
int conn_poll_run(conn_registry_t *registry) {
	...
	while ((count = conn_poll_prepare(registry, &pfds, &next_timestamp)) > 0) {
		...
		if (timediff < 0)
			timediff = 0;
#ifdef __vita__
		// vitasdk's poll() does not wake on incoming socket data during a long
		// wait; it only reports POLLIN reliably with short timeouts. Cap the
		// wait so the socket is drained promptly (during ICE the timeout is
		// naturally tiny and receiving works; after the connection goes idle it
		// would otherwise jump to seconds and packets would sit unread).
		if (timediff > 20)
			timediff = 20;
#endif
