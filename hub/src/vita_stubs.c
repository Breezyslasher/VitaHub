 ...
 * size of 512KB.
 */
#define VITAHUB_MIN_THREAD_STACK (512 * 1024)
...
/*
 * getentropy() syscall glue (from Vita_abs).
 *
 * Current vitasdk newlib ships a getentropy() wrapper that calls the
 * reentrant syscall _getentropy_r(), but not every SDK release defines it,
 * and mbedTLS 3.6.7 calls getentropy() for its entropy source. Weak, so an
 * SDK that does provide it wins.
 */
#include <errno.h>
#include <sys/reent.h>
#include <psp2/kernel/rng.h>

__attribute__((weak)) int _getentropy_r(struct _reent *reent, void *buf, size_t buflen) {
