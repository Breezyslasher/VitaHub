/*
    VitaHub — PS4 in-app self-update
*/
...
namespace vitahub { namespace ps4 {
...
/// Launch the separate updater helper app (title VHUB00003), which installs
/// the downloaded pkg (/data/VitaHub/update.pkg, by convention) while
/// VitaHub is closed — the only way to replace the running title, since the
/// system won't install over it in place. VitaHub must quit right after a
/// successful launch so the helper can take over.
///
/// Returns 0 if the helper was launched (VitaHub should now quit); non-zero
...
/// Install the updater helper app, whose pkg ships inside VitaHub's own pkg
...
/// Remove the updater helper if it's installed. Call this from VitaHub once an
/// update has completed — the helper is only needed while one installs, and
/// VitaHub reinstalls it on demand. Cleaning up from here rather than letting
