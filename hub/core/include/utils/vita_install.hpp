...
/// off to the updater stub, and by the stub to relaunch VitaHub afterwards.
void launchTitle(const std::string& titleId);

/// Uninstall the updater stub (title VHUBUPD01) if it's installed, via
/// ScePromoterUtility. Call from VitaHub on startup after an update: the stub
