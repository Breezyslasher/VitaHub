 ...
 *
 * Supported platforms:
 *   PS Vita  – ux0:data/VitaHub
 *   Switch   – sdmc:/VitaHub
 *   Android  – <SDL internal storage>/VitaHub  (resolved at runtime)
 *   Desktop  – ./VitaHub  (next to the executable)
 */
...
namespace vitahub {
#if defined(__vita__)
    static constexpr const char* PLATFORM_DATA_DIR = "ux0:data/VitaHub";
#elif defined(__SWITCH__)
    static constexpr const char* PLATFORM_DATA_DIR = "sdmc:/VitaHub";
    ...
    static constexpr const char* PLATFORM_DATA_DIR = "/data/VitaHub";
    ...
    static constexpr const char* PLATFORM_DATA_DIR = "";
#else
    // Desktop: $XDG_DATA_HOME/VitaHub, per the XDG Base Directory spec, which
    ...
    inline const std::string& getDesktopDataDir() {
        ...
        if (s_dir.empty()) {
            ...
            // fell all the way through to "./VitaHub" — a path relative to the
            ...
            // losing them. So a "VitaHub" directory that is already there wins,
            // and only fresh installs get the correct location.
            std::error_code ec;
            const char* localApp = std::getenv("LOCALAPPDATA");
            if (std::filesystem::is_directory("VitaHub", ec)) {
                s_dir = "./VitaHub";
            } else if (localApp && *localApp) {
                s_dir = std::string(localApp) + "\\VitaHub";
            } else {
                s_dir = "./VitaHub";
            }
#else
            ...
            if (xdgData && xdgData[0] == '/') {
                s_dir = std::string(xdgData) + "/VitaHub";
            } else if (home && *home) {
                s_dir = std::string(home) + "/.local/share/VitaHub";
            } else {
                s_dir = "./VitaHub";
            }
#endif
 ...
 * Example: /data/user/0/org.VitaPlex.app/files/VitaHub
 */
...
inline void setAndroidDataDir(const std::string& filesDir) {
    ...
    if (dir.empty()) dir = filesDir + "/VitaHub";
}
...
inline const std::string& getAndroidDataDir() {
    ...
    if (s_dataDir.empty()) {
        ...
        if (internalPath && internalPath[0] != '\0') {
            s_dataDir = std::string(internalPath) + "/VitaHub";
        } else {
            // Absolute fallback — should never happen in practice
            s_dataDir = "/sdcard/VitaHub";
        }
    }
    return s_dataDir;
}
 ...
 * Example: platformPath("downloads") -> "ux0:data/VitaHub/downloads"
 *
