/*
    VitaHub — updater stub

    A tiny standalone Vita app whose ONLY job is to install a VitaHub update
    while VitaHub itself is closed, then relaunch it. This is the AutoPlugin2
    technique: a title cannot promote over itself (the Vita installer returns
    0x80101114 "in use" for the running app), so VitaHub installs and launches
    THIS stub, which — now that VitaHub is no longer the running title — can
    promote VitaHub's downloaded VPK cleanly, then boots VitaHub.

    Everything is fixed by convention so no arguments need to cross the app
    boundary:
      - the VPK to install : ux0:data/VitaHub/update.vpk
      - the target version : ux0:data/VitaHub/update_version.txt (optional)
      - the app to relaunch : VHUB00001 (VitaHub's title id)

    The heavy lifting (extract + head.bin forge + promote) is shared verbatim
    with VitaHub via utils/vita_install, which is deliberately free of any
...
const char* kUpdateVpk  = "ux0:data/VitaHub/update.vpk";
const char* kVersionTxt = "ux0:data/VitaHub/update_version.txt";
const char* kWorkDir    = "ux0:data/VitaHub/updater_work";
const char* kTargetId   = "VHUB00001";
...
void log(const char* msg) {
    SceUID fd = sceIoOpen("ux0:data/VitaHub/updater.log",
...
void render() {
    ...
    if (!g_ui.version.empty()) {
        ...
        drawWrapped(46, 316, 308, 0.85f, C_MUTED,
                    "VitaHub will restart automatically when this finishes.", 22);
    } else {
        drawWrapped(46, 294, 308, 0.85f, C_MUTED,
                    "Installing the update. VitaHub will restart automatically "
...
int main(int, char*[]) {
    ...
    // Give the shell a moment to finish tearing down VitaHub so the promoter no longer sees the title as in use.
    ...
    if (rc == 0) {
        ...
        g_ui.note = "Update kept at ux0:data/VitaHub/update.vpk - install it with VitaShell";
        render();
        sceKernelDelayThread(4 * 1000 * 1000);
    }

    // Relaunch VitaHub either way — the updated build on success, the
