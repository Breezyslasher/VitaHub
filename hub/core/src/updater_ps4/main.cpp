/*
    VitaHub — PS4 updater helper

    A tiny standalone PS4 app whose ONLY job is to install a VitaHub update
    while VitaHub itself is closed. This is the PS4 analogue of the Vita
    ...
    process. So VitaHub launches THIS helper and quits; with VitaHub no
    longer running, the helper uninstalls VHUB00002 (its data under
    /data/VitaHub is on a different partition and is untouched) and hands the
    ...
    in the notifications / Downloads list, and the user reopens VitaHub once it
    ...
    Everything is fixed by convention so no arguments cross the app boundary:
      - the pkg to install : /data/VitaHub/update.pkg
      - the app to replace : VHUB00002 (VitaHub's title id)
...
// Relaunching VitaHub when the install finishes. Layout per idc's
...
const char* kPkgPath  = "/data/VitaHub/update.pkg";
...
const char* kPkgPathSystem = "/user/data/VitaHub/update.pkg";
const char* kLogPath  = "/data/VitaHub/updater_ps4.log";
const char* kTargetId = "VHUB00002";

constexpr size_t kBgftHeapSize  = 1 * 1024 * 1024;
// SCE_BGFT_TASK_OPT_FORCE_UPDATE, per flatz's bgft.h. VitaHub is closed while
...
void uiInit() {
    ...
    // borealis demonstrably renders on this hardware (VitaHub logs
    ...
    g_win = SDL_CreateWindow("VitaHub Updater", SDL_WINDOWPOS_UNDEFINED,
...
int main(int, char*[]) {
    ...
    // nothing. VitaHub makes these same calls successfully because borealis
    // has already run this.
    int sdl = SDL_Init(SDL_INIT_VIDEO);
    vlog("ps4 updater: SDL_Init -> %d (%s)", sdl, sdl == 0 ? "ok" : SDL_GetError());

    // Give VitaHub a moment to fully exit before we uninstall its title.
    sleep(3);
    ...
    // launched by a quitting VitaHub never becomes the foreground/composited
    ...
    // the same way for a sandboxed app, and VitaHub only gets away with direct
    ...
    // Optional extras: used to wait for the install and relaunch VitaHub. If
    ...
    paramsEx.param.contentName      = "VitaHub";
    ...
    // console with no VitaHub at all — which is exactly what happened when the
    ...
    // VitaHub, after proving on-device that both fight the platform:
    ...
    //   * Relaunching VHUB00002 the moment the transfer starts launches a title
    //     that was just uninstalled and is still being written, which the
    //     system reports as a crash (CE-36329-3).
    // The user reopens VitaHub from the home screen once the install
    // notification finishes — the same flow every other PS4 homebrew uses.
    // VitaHub removes this helper on its next start (ps4::removeUpdaterApp).
    sleep(2);
    vlog("ps4 updater: done (install continues in the background; reopen "
         "VitaHub when the system notification finishes)");
    return 0;
}
