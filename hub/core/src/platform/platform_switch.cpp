/**
 * VitaHub platform layer — Nintendo Switch (libnx) implementation.
 *
...
// The display mode is the OS/console's business on this port; VitaHub neither
...
void shutdown() {
    closeLogFile();
    ::vitahub::HttpClient::globalCleanup();
}
...
std::string getLogPath() {
    ...
    // sdmc:/VitaHub/vitahub.log so users can ship the file back to
    // us when something goes wrong (the empty sdmc:/VitaHub/ dir was
    // the immediate diagnostic for the "failed to connect to OMV" report).
    return "sdmc:/VitaHub/vitahub.log";
}
