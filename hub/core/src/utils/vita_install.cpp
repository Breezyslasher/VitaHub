/*
    VitaHub — PS Vita in-app self-update (ported from pleNx, thcolin/gamepad-media-center-aggregator)
    ...
    libmpv, so the distributed VitaHub.vpk is likewise a GPL combined work (Vita build links GPL libmpv) and this reuse
...
// VitaHub (which has borealis) and the tiny updater-stub app (which does not),
...
int promoteApp(const std::string& path, std::string& err,
    ...
    if (res >= 0) {
        // VitaHub ships with an empty CONTENT_ID (free homebrew, no
...
namespace vitahub { namespace vita {
