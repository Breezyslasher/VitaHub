/**
 * VitaHub - In-app updates implementation
 *
 * Ported from pleNx (thcolin/gamepad-media-center-aggregator,
 * app/src/utils/version.cpp) onto VitaHub's HttpClient and dialogs.
...
#define VITAHUB_MACOS_DESKTOP 1
#else
#define VITAHUB_MACOS_DESKTOP 0
#endif
...
constexpr const char* kRepo = "Breezyslasher/VitaHub";
...
//   * Deb      : dpkg lays the binary at /usr/lib/VitaHub/VitaHub.
...
LinuxPkg linuxPkg() {
    ...
    if (exe.rfind("/usr/lib/VitaHub/", 0) == 0 &&
...
// The Windows build ships as a portable folder — VitaHub.exe + its runtime
...
// the zip, then hand a tiny .bat to a detached cmd that waits for VitaHub to
...
// The macOS build ships as VitaHub.app inside a .dmg. In-app update mounts
// the downloaded dmg and swaps the whole .app bundle from a detached helper
// once VitaHub has quit, then reopens it — the classic Sparkle shape.
...
std::string macAppBundlePath() {
    ...
    const std::string tail = "/Contents/MacOS/VitaHub";
...
#if defined(__SWITCH__) || defined(__PSV__) || defined(ANDROID) || defined(__PS4__) || (defined(__linux__) && !defined(ANDROID)) || defined(_WIN32) || VITAHUB_MACOS_DESKTOP
...
// brls::Application::quit() does an orderly shutdown, but VitaHub keeps live
...
// the helper, after its short startup wait, uninstalls VHUB00002. Uninstalling
// a title whose process is still running is reported as a crash (CE-36329-3),
// which is what took VitaHub down on the system-UI build. _Exit() ends the
// process at once — no destructors, no thread joins, no lingering title — so
// the helper's uninstall always lands on an already-gone VitaHub. The update
...
void startInstall(const ReleaseInfo rel) {
    ...
    auto* keepOpen = makeLabel("Keep VitaHub open until this finishes", 11.0f, tok::disabled());
...
#elif defined(_WIN32) || VITAHUB_MACOS_DESKTOP
    const char* relaunchLabel = "Reopens automatically";
#else
    ...
    asyncRun([rel, ui]() {
        ...
        // (VHUB00003): with VitaHub closed it uninstalls the old title and
                ...
                if (ps4::installUpdaterApp(setupErr) == 0) {
                    ...
                    if (launched) {
                        ...
                        finishInstall(ui, [path]() {
                            auto* d = new brls::Dialog(
                                ...
                                "or close VitaHub and install that .pkg with "
                    ...
                    finishInstall(ui, [path, setupErr]() {
                        auto* d = new brls::Dialog(
                            ...
                            "VitaHub and install that .pkg with Itemzflow (or "
        ...
        if (target.size() < 4 || target.compare(target.size() - 4, 4, ".nro") != 0)
            target = platformPath("VitaHub.nro");
        std::error_code ec;
        ...
        if (linuxPkg() == LinuxPkg::AppImage) {
            ...
            finishInstall(ui, [path]() {
                ...
                // can't, so the user reopens VitaHub once the system install
                // finishes.
                pid_t pid = fork();
                if (pid == 0) {
                    setsid();
...
#elif defined(_WIN32)
        // Windows portable build. The running VitaHub.exe and its loaded
        // DLLs are locked, so the swap waits until we exit: write a .bat that
        // spins until VitaHub.exe is gone, unpacks the downloaded zip over
        ...
        // it to a detached, windowless cmd and quit.
        {
            ...
            const std::string batPath = installDir + "\\vitahub_update.bat";
            ...
            fprintf(bf,
                "@echo off\r\n"
                ":waitloop\r\n"
                "tasklist /FI \"IMAGENAME eq VitaHub.exe\" 2>nul | find /I \"VitaHub.exe\" >nul\r\n"
                "if not errorlevel 1 (\r\n"
...
#elif VITAHUB_MACOS_DESKTOP
        // macOS .app swap, run from a detached shell after VitaHub quits:
        // wait for the process to die, mount the dmg, ditto its VitaHub.app
        ...
        // prompt is involved.
        {
            ...
            // VitaHub data paths carry no single quotes, so single-quoting each is enough to keep the shell happy.
            fprintf(sf,
                "#!/bin/sh\n"
                "PID=%d\n"
                "DMG='%s'\n"
...
#else
        // VitaHub can't promote over itself while it is the running title
        // (the installer returns 0x80101114 "in use"), so hand the install
        // to a tiny bundled stub app — the AutoPlugin2 technique. VitaHub
        // installs the stub (the stub isn't running, so promoting IT is
        // fine), launches it, and quits; with VitaHub closed the stub
        // promotes the downloaded update.vpk and relaunches VitaHub.
        brls::sync([ui]() {
            if (!ui->dismissed->load())
        ...
        // stub promotes the VPK and reopens VitaHub. (The Vita OS still shows
...
// VitaHub notes are hand-written markdown with a fixed shape: an H1
...
void offerUpdate(const ReleaseInfo rel) {
    ...
    auto* sub = makeLabel("A new version of VitaHub is ready to install.", 11.5f, tok::muted());
    ...
    cards->addView(makeCard("CURRENT", tok::muted2(), VITAHUB_DISPLAY_VERSION,
...
void checkForUpdates(bool manual) {
    ...
    asyncRun([manual]() {
        ...
        // only needed while an update installs, and VitaHub reinstalls it on
        ...
        // (CE-36329-3). From VitaHub it's just another title, so it goes
        // quietly.
        ps4::removeUpdaterApp();
#endif
#ifdef __PSV__
        // Same idea on Vita: the updater stub (VHUBUPD01) is installed fresh
        ...
        HttpClient client;
        HttpRequest req;
        // VitaHub publishes pre-releases; /releases/latest never returns
        ...
        const std::string current = VITAHUB_DISPLAY_VERSION;
        if (!isNewer(rel.tag, current)) {
            brls::Logger::info("app_update: up to date ({} vs {})", current, rel.tag);
            if (manual) {
                brls::sync([current]() {
                    auto* d = new brls::Dialog("VitaHub is up to date (" + current + ").");
                    d->addButton("OK", []() {});
                    d->open();
                });
            }
