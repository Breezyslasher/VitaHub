/*
    ...
    VitaHub itself. PS4-Store clears this by uninstalling the title and
...
constexpr const char* kUpdaterTitle = "VHUB00003";
...
// instantly (it took VitaHub down with CE-34878-0 when sceAppInstUtilAppExists
...
int installPkg(const std::string& pkgPath, const std::string& contentName, std::string& err) {
    ...
    // title is the running VitaHub itself. We deliberately do NOT uninstall
...
int installUpdaterApp(std::string& err) {
    // The helper's pkg ships inside VitaHub's own pkg (see CMakeLists), so the
    ...
    // VitaHub, so this install is not blocked by 0x80990088.
    const std::string src = "/app0/updater.pkg";
    const std::string dst = "/data/VitaHub/updater.pkg";
    ...
    return installPkg(dst, "VitaHub Updater", err);
}
