; VitaHub Windows installer.
;
; Per-user by design, into %LOCALAPPDATA%\Programs\VitaHub. That choice is not
...
; privileged helper, and VitaHub updates itself. Installing per-user means no
...
!endif
!ifndef OUTFILE
  !define OUTFILE "VitaHub-Setup.exe"
!endif

!define APPNAME     "VitaHub"
...
!endif
!define MUI_ICON   "${ICONDIR}\vitahub.ico"
!define MUI_UNICON "${ICONDIR}\vitahub.ico"

!insertmacro MUI_PAGE_DIRECTORY
!insertmacro MUI_PAGE_INSTFILES
!define MUI_FINISHPAGE_RUN "$INSTDIR\VitaHub.exe"
...
Function ${un}EnsureNotRunning
  FindWindow $0 "" "VitaHub"
  StrCmp $0 0 done
    MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION \
      "VitaHub is running. Close it and press OK to continue." IDOK done
    Abort
  done:
FunctionEnd
!macroend
...
Section "Install"
  ...
  CreateShortcut "$SMPROGRAMS\${APPNAME}.lnk" "$INSTDIR\VitaHub.exe" "" \
                 "$INSTDIR\VitaHub.exe" 0 SW_SHOWNORMAL "" "${APPNAME}"
  ...
  WriteRegStr   HKCU "${REGKEY}" "DisplayIcon"     "$INSTDIR\VitaHub.exe,0"
...
Section "Uninstall"
  ...
  ; Settings, downloads and cache live in %LOCALAPPDATA%\VitaHub and are
