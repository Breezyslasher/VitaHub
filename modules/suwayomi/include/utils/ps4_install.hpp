/*
    VitaSuwayomi — PS4 in-app self-update
*/
...
namespace vitasuwayomi { namespace ps4 {
...
/// the downloaded pkg (/data/VitaSuwayomi/update.pkg, by convention) while
/// VitaSuwayomi is closed — the only way to replace the running title, since the
/// system won't install over it in place. VitaSuwayomi must quit right after a
/// successful launch so the helper can take over.
///
/// Returns 0 if the helper was launched (VitaSuwayomi should now quit); non-zero
...
/// Install the updater helper app, whose pkg ships inside VitaSuwayomi's own pkg
...
/// Remove the updater helper if it's installed. Call this from VitaSuwayomi once an
/// update has completed — the helper is only needed while one installs, and
/// VitaSuwayomi reinstalls it on demand. Cleaning up from here rather than letting
