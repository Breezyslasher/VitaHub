...
void Application::start() {
    ...
    if (!m_serverUrl.empty()) {
        ...
        // This prevents a multi-minute freeze on PS4 (and improves UX everywhere)
        // when the server is unreachable and each network call hits a 30s timeout.
        pushLoginActivity();

        std::string serverUrl = m_serverUrl;
        asyncRun([this, serverUrl]() {
            brls::Logger::info("Restoring saved connection (async)...");
            ...
            if (authValid) {
                ...
                // Apply server image settings on first run
                ...
                // Transition to main activity on the UI thread
                brls::sync([this]() {
                    ...
                    // Replace the login activity with main activity
                    pushMainActivity();
                });
            } else {
                ...
                brls::sync([this]() {
                    ...
                    // Otherwise stay on login screen (already showing)
                });
            }
        });
    } else {
...
static const ThemePalette& getPalette(AppTheme theme) {
    ...
    static const ThemePalette tachiyomiPalette = {
        nvgRGB(0xB0, 0xC6, 0xFF),           // accent
        nvgRGB(0x7A, 0xDC, 0x77),           // secondary
        nvgRGB(0xB0, 0xC6, 0xFF),           // headerText
        nvgRGB(0xC5, 0xC6, 0xD0),           // subtitle
        nvgRGBA(0xB0, 0xC6, 0xFF, 80),      // highlight
        nvgRGBA(0x1E, 0x1C, 0x22, 255),     // sidebar
        nvgRGBA(0x21, 0x1F, 0x26, 255),     // cardBg
        nvgRGBA(0x1A, 0x18, 0x1D, 255),     // dialogBg
        nvgRGB(0x7A, 0xDC, 0x77),           // teal
        nvgRGB(0xB0, 0xC6, 0xFF),           // status
        nvgRGB(0xC5, 0xC6, 0xD0),           // description
        nvgRGB(0x8F, 0x90, 0x99),           // dimText
        nvgRGBA(0x21, 0x1F, 0x26, 255),     // rowBg
        nvgRGBA(0xB0, 0xC6, 0xFF, 90),      // activeRowBg
        nvgRGBA(0x29, 0x27, 0x30, 200),     // inactiveRowBg
        nvgRGBA(0xB0, 0xC6, 0xFF, 255),     // trackingBtn
        nvgRGBA(0xB0, 0xC6, 0xFF, 255),     // ctaButton
        nvgRGB(0x30, 0x2E, 0x38),           // sectionHeaderBg
        nvgRGBA(0x1A, 0x18, 0x1D, 255),     // readerBg
        nvgRGBA(0x1A, 0x18, 0x1D, 200),     // errorOverlayBg
        nvgRGBA(0x30, 0x2E, 0x38, 255),     // focusedRowBg
        nvgRGBA(0x1A, 0x18, 0x1D, 255),     // deepBg
        nvgRGBA(0xE3, 0xE2, 0xE6, 100),     // separatorColor
        nvgRGB(0x7A, 0xDC, 0x77),           // successText
        nvgRGB(0xE3, 0xE2, 0xE6),           // textColor
        nvgRGBA(0x29, 0x27, 0x30, 255),     // buttonColor
    };

    // Neon Vaporwave
    static const ThemePalette neonVaporwavePalette = {
        nvgRGB(0xFF, 0x32, 0xC8),           // accent
        nvgRGB(0x00, 0xFF, 0xC8),           // secondary
        nvgRGB(0xFF, 0x32, 0xC8),           // headerText
        nvgRGB(0xC8, 0xB4, 0xFF),           // subtitle
        nvgRGBA(0xFF, 0x32, 0xC8, 80),      // highlight
        nvgRGBA(0x14, 0x00, 0x28, 255),     // sidebar
        nvgRGBA(0x1E, 0x05, 0x32, 255),     // cardBg
        nvgRGBA(0x0F, 0x00, 0x1E, 255),     // dialogBg
        nvgRGB(0x00, 0xFF, 0xC8),           // teal
        nvgRGB(0xFF, 0x32, 0xC8),           // status
        nvgRGB(0xC8, 0xB4, 0xFF),           // description
        nvgRGB(0x8C, 0x64, 0xB4),           // dimText
        nvgRGBA(0x1E, 0x05, 0x32, 255),     // rowBg
        nvgRGBA(0xFF, 0x32, 0xC8, 90),      // activeRowBg
        nvgRGBA(0x32, 0x0A, 0x50, 200),     // inactiveRowBg
        nvgRGBA(0xFF, 0x32, 0xC8, 255),     // trackingBtn
        nvgRGBA(0xFF, 0x32, 0xC8, 255),     // ctaButton
        nvgRGB(0x3C, 0x0F, 0x5A),           // sectionHeaderBg
        nvgRGBA(0x0F, 0x00, 0x1E, 255),     // readerBg
        nvgRGBA(0x0F, 0x00, 0x1E, 200),     // errorOverlayBg
        nvgRGBA(0x3C, 0x0F, 0x5A, 255),     // focusedRowBg
        nvgRGBA(0x0F, 0x00, 0x1E, 255),     // deepBg
        nvgRGBA(0xFF, 0xFF, 0xFF, 100),     // separatorColor
        nvgRGB(0x00, 0xFF, 0xC8),           // successText
        nvgRGB(0xFF, 0xFF, 0xFF),           // textColor
        nvgRGBA(0x32, 0x0A, 0x50, 255),     // buttonColor
    };

    // Catppuccin
    static const ThemePalette catppuccinPalette = {
        nvgRGB(0xCB, 0xA6, 0xF7),           // accent
        nvgRGB(0x94, 0xE2, 0xD5),           // secondary
        nvgRGB(0xCB, 0xA6, 0xF7),           // headerText
        nvgRGB(0xBA, 0xC2, 0xDE),           // subtitle
        nvgRGBA(0xCB, 0xA6, 0xF7, 80),      // highlight
        nvgRGBA(0x1E, 0x1E, 0x2E, 255),     // sidebar
        nvgRGBA(0x1E, 0x1E, 0x2E, 255),     // cardBg
        nvgRGBA(0x18, 0x18, 0x25, 255),     // dialogBg
        nvgRGB(0x94, 0xE2, 0xD5),           // teal
        nvgRGB(0xCB, 0xA6, 0xF7),           // status
        nvgRGB(0xBA, 0xC2, 0xDE),           // description
        nvgRGB(0x7F, 0x84, 0x9C),           // dimText
        nvgRGBA(0x1E, 0x1E, 0x2E, 255),     // rowBg
        nvgRGBA(0xCB, 0xA6, 0xF7, 90),      // activeRowBg
        nvgRGBA(0x31, 0x32, 0x44, 200),     // inactiveRowBg
        nvgRGBA(0xCB, 0xA6, 0xF7, 255),     // trackingBtn
        nvgRGBA(0xCB, 0xA6, 0xF7, 255),     // ctaButton
        nvgRGB(0x31, 0x32, 0x44),           // sectionHeaderBg
        nvgRGBA(0x18, 0x18, 0x25, 255),     // readerBg
        nvgRGBA(0x18, 0x18, 0x25, 200),     // errorOverlayBg
        nvgRGBA(0x31, 0x32, 0x44, 255),     // focusedRowBg
        nvgRGBA(0x18, 0x18, 0x25, 255),     // deepBg
        nvgRGBA(0xCD, 0xD6, 0xF4, 100),     // separatorColor
        nvgRGB(0xA6, 0xE3, 0xA1),           // successText
        nvgRGB(0xCD, 0xD6, 0xF4),           // textColor
        nvgRGBA(0x31, 0x32, 0x44, 255),     // buttonColor
    };

    // Nord
    static const ThemePalette nordPalette = {
        nvgRGB(0x88, 0xC0, 0xD0),           // accent
        nvgRGB(0x5E, 0x81, 0xAC),           // secondary
        nvgRGB(0x88, 0xC0, 0xD0),           // headerText
        nvgRGB(0xD8, 0xDE, 0xE9),           // subtitle
        nvgRGBA(0x88, 0xC0, 0xD0, 80),      // highlight
        nvgRGBA(0x3B, 0x42, 0x52, 255),     // sidebar
        nvgRGBA(0x43, 0x4C, 0x5E, 255),     // cardBg
        nvgRGBA(0x2E, 0x34, 0x40, 255),     // dialogBg
        nvgRGB(0x5E, 0x81, 0xAC),           // teal
        nvgRGB(0x88, 0xC0, 0xD0),           // status
        nvgRGB(0xD8, 0xDE, 0xE9),           // description
        nvgRGB(0x6D, 0x71, 0x7B),           // dimText
        nvgRGBA(0x43, 0x4C, 0x5E, 255),     // rowBg
        nvgRGBA(0x88, 0xC0, 0xD0, 90),      // activeRowBg
        nvgRGBA(0x4C, 0x56, 0x6A, 200),     // inactiveRowBg
        nvgRGBA(0x88, 0xC0, 0xD0, 255),     // trackingBtn
        nvgRGBA(0x88, 0xC0, 0xD0, 255),     // ctaButton
        nvgRGB(0x50, 0x59, 0x68),           // sectionHeaderBg
        nvgRGBA(0x2E, 0x34, 0x40, 255),     // readerBg
        nvgRGBA(0x2E, 0x34, 0x40, 200),     // errorOverlayBg
        nvgRGBA(0x50, 0x59, 0x68, 255),     // focusedRowBg
        nvgRGBA(0x2E, 0x34, 0x40, 255),     // deepBg
        nvgRGBA(0xEC, 0xEF, 0xF4, 100),     // separatorColor
        nvgRGB(0xA3, 0xBE, 0x8C),           // successText
        nvgRGB(0xEC, 0xEF, 0xF4),           // textColor
        nvgRGBA(0x4C, 0x56, 0x6A, 255),     // buttonColor
    };

    // Tako
    static const ThemePalette takoPalette = {
        nvgRGB(0xF3, 0xB3, 0x75),           // accent
        nvgRGB(0x66, 0x57, 0x7E),           // secondary
        nvgRGB(0xF3, 0xB3, 0x75),           // headerText
        nvgRGB(0xCB, 0xC4, 0xCE),           // subtitle
        nvgRGBA(0xF3, 0xB3, 0x75, 80),      // highlight
        nvgRGBA(0x26, 0x26, 0x36, 255),     // sidebar
        nvgRGBA(0x2A, 0x2A, 0x3C, 255),     // cardBg
        nvgRGBA(0x20, 0x20, 0x2E, 255),     // dialogBg
        nvgRGB(0x66, 0x57, 0x7E),           // teal
        nvgRGB(0xF3, 0xB3, 0x75),           // status
        nvgRGB(0xCB, 0xC4, 0xCE),           // description
        nvgRGB(0x95, 0x8F, 0x99),           // dimText
        nvgRGBA(0x2A, 0x2A, 0x3C, 255),     // rowBg
        nvgRGBA(0xF3, 0xB3, 0x75, 90),      // activeRowBg
        nvgRGBA(0x30, 0x30, 0x44, 200),     // inactiveRowBg
        nvgRGBA(0xF3, 0xB3, 0x75, 255),     // trackingBtn
        nvgRGBA(0xF3, 0xB3, 0x75, 255),     // ctaButton
        nvgRGB(0x36, 0x36, 0x4D),           // sectionHeaderBg
        nvgRGBA(0x20, 0x20, 0x2E, 255),     // readerBg
        nvgRGBA(0x20, 0x20, 0x2E, 200),     // errorOverlayBg
        nvgRGBA(0x36, 0x36, 0x4D, 255),     // focusedRowBg
        nvgRGBA(0x20, 0x20, 0x2E, 255),     // deepBg
        nvgRGBA(0xE3, 0xE0, 0xF2, 100),     // separatorColor
        nvgRGB(0x9E, 0xD3, 0x6A),           // successText
        nvgRGB(0xE3, 0xE0, 0xF2),           // textColor
        nvgRGBA(0x30, 0x30, 0x44, 255),     // buttonColor
    };

    // Midnight Dusk
    static const ThemePalette midnightDuskPalette = {
        nvgRGB(0xF0, 0x24, 0x75),           // accent
        nvgRGB(0x55, 0x97, 0x1C),           // secondary
        nvgRGB(0xF0, 0x24, 0x75),           // headerText
        nvgRGB(0xD6, 0xC1, 0xC4),           // subtitle
        nvgRGBA(0xF0, 0x24, 0x75, 80),      // highlight
        nvgRGBA(0x25, 0x15, 0x22, 255),     // sidebar
        nvgRGBA(0x28, 0x16, 0x24, 255),     // cardBg
        nvgRGBA(0x16, 0x15, 0x1D, 255),     // dialogBg
        nvgRGB(0x55, 0x97, 0x1C),           // teal
        nvgRGB(0xF0, 0x24, 0x75),           // status
        nvgRGB(0xD6, 0xC1, 0xC4),           // description
        nvgRGB(0x9F, 0x8C, 0x8F),           // dimText
        nvgRGBA(0x28, 0x16, 0x24, 255),     // rowBg
        nvgRGBA(0xF0, 0x24, 0x75, 90),      // activeRowBg
        nvgRGBA(0x2D, 0x1C, 0x2A, 200),     // inactiveRowBg
        nvgRGBA(0xF0, 0x24, 0x75, 255),     // trackingBtn
        nvgRGBA(0xF0, 0x24, 0x75, 255),     // ctaButton
        nvgRGB(0x2F, 0x1F, 0x2C),           // sectionHeaderBg
        nvgRGBA(0x16, 0x15, 0x1D, 255),     // readerBg
        nvgRGBA(0x16, 0x15, 0x1D, 200),     // errorOverlayBg
        nvgRGBA(0x2F, 0x1F, 0x2C, 255),     // focusedRowBg
        nvgRGBA(0x16, 0x15, 0x1D, 255),     // deepBg
        nvgRGBA(0xE5, 0xE1, 0xE5, 100),     // separatorColor
        nvgRGB(0x55, 0x97, 0x1C),           // successText
        nvgRGB(0xE5, 0xE1, 0xE5),           // textColor
        nvgRGBA(0x2D, 0x1C, 0x2A, 255),     // buttonColor
    };

    // Green Apple
    static const ThemePalette greenApplePalette = {
        nvgRGB(0x7A, 0xDB, 0x8F),           // accent
        nvgRGB(0xFF, 0xB3, 0xAC),           // secondary
        nvgRGB(0x7A, 0xDB, 0x8F),           // headerText
        nvgRGB(0xBE, 0xCA, 0xBC),           // subtitle
        nvgRGBA(0x7A, 0xDB, 0x8F, 80),      // highlight
        nvgRGBA(0x18, 0x1D, 0x18, 255),     // sidebar
        nvgRGBA(0x1C, 0x21, 0x1C, 255),     // cardBg
        nvgRGBA(0x0A, 0x0F, 0x0B, 255),     // dialogBg
        nvgRGB(0xFF, 0xB3, 0xAC),           // teal
        ...
        nvgRGBA(0x7A, 0xDB, 0x8F, 90),      // activeRowBg
        nvgRGBA(0x26, 0x2B, 0x26, 200),     // inactiveRowBg
        nvgRGBA(0x7A, 0xDB, 0x8F, 255),     // trackingBtn
        nvgRGBA(0x7A, 0xDB, 0x8F, 255),     // ctaButton
        nvgRGB(0x31, 0x36, 0x30),           // sectionHeaderBg
        nvgRGBA(0x0A, 0x0F, 0x0B, 255),     // readerBg
        nvgRGBA(0x0A, 0x0F, 0x0B, 200),     // errorOverlayBg
        nvgRGBA(0x31, 0x36, 0x30, 255),     // focusedRowBg
        nvgRGBA(0x0A, 0x0F, 0x0B, 255),     // deepBg
        nvgRGBA(0xDF, 0xE4, 0xDB, 100),     // separatorColor
        nvgRGB(0xFF, 0xB3, 0xAC),           // successText
    ...
    static const ThemePalette lavenderPalette = {
        ...
        nvgRGBA(0xA1, 0x77, 0xFF, 90),      // activeRowBg
        ...
        nvgRGBA(0x15, 0x13, 0x2D, 255),     // readerBg
        nvgRGBA(0x15, 0x13, 0x2D, 200),     // errorOverlayBg
        nvgRGBA(0x28, 0x24, 0x46, 255),     // focusedRowBg
        nvgRGBA(0x15, 0x13, 0x2D, 255),     // deepBg
        nvgRGBA(0xE7, 0xE0, 0xEC, 100),     // separatorColor
    ...
    static const ThemePalette matrixPalette = {
        nvgRGB(0x00, 0xFF, 0x00),           // accent
        nvgRGB(0xFF, 0xFF, 0xFF),           // secondary
        nvgRGB(0x00, 0xFF, 0x00),           // headerText
        nvgRGB(0x00, 0xCC, 0x00),           // subtitle
        nvgRGBA(0x00, 0xFF, 0x00, 80),      // highlight
        nvgRGBA(0x0A, 0x0A, 0x0A, 255),     // sidebar
        nvgRGBA(0x21, 0x21, 0x21, 255),     // cardBg
        nvgRGBA(0x08, 0x08, 0x08, 255),     // dialogBg
        nvgRGB(0xFF, 0xFF, 0xFF),           // teal
        nvgRGB(0x00, 0xFF, 0x00),           // status
        nvgRGB(0x00, 0xCC, 0x00),           // description
        nvgRGB(0x00, 0x88, 0x00),           // dimText
        nvgRGBA(0x21, 0x21, 0x21, 255),     // rowBg
        nvgRGBA(0x00, 0xFF, 0x00, 90),      // activeRowBg
        nvgRGBA(0x21, 0x21, 0x21, 200),     // inactiveRowBg
        nvgRGBA(0x00, 0xFF, 0x00, 255),     // trackingBtn
        nvgRGBA(0x00, 0xFF, 0x00, 255),     // ctaButton
        nvgRGB(0x2A, 0x2A, 0x2A),           // sectionHeaderBg
        nvgRGBA(0x08, 0x08, 0x08, 255),     // readerBg
        nvgRGBA(0x08, 0x08, 0x08, 200),     // errorOverlayBg
    ...
    static const ThemePalette mochaPalette = {
        ...
        nvgRGBA(0xFB, 0xB7, 0x7F, 90),      // activeRowBg
        nvgRGBA(0x31, 0x28, 0x22, 200),     // inactiveRowBg
        nvgRGBA(0xFB, 0xB7, 0x7F, 255),     // trackingBtn
        nvgRGBA(0xFB, 0xB7, 0x7F, 255),     // ctaButton
        nvgRGB(0x3C, 0x33, 0x2D),           // sectionHeaderBg
        nvgRGBA(0x14, 0x0D, 0x08, 255),     // readerBg
        nvgRGBA(0x14, 0x0D, 0x08, 200),     // errorOverlayBg
        nvgRGBA(0x3C, 0x33, 0x2D, 255),     // focusedRowBg
        nvgRGBA(0x14, 0x0D, 0x08, 255),     // deepBg
        nvgRGBA(0xF0, 0xDF, 0xD6, 100),     // separatorColor
        nvgRGB(0xAE, 0xD1, 0x8D),           // successText
        nvgRGB(0xF0, 0xDF, 0xD6),           // textColor
        nvgRGBA(0x31, 0x28, 0x22, 255),     // buttonColor
    };

    // Sunset
    static const ThemePalette sunsetPalette = {
        nvgRGB(0xFF, 0x8A, 0x5C),           // accent
        nvgRGB(0xFF, 0xC1, 0x5B),           // secondary
        nvgRGB(0xFF, 0x8A, 0x5C),           // headerText
        nvgRGB(0xD8, 0xC3, 0xBA),           // subtitle
        nvgRGBA(0xFF, 0x8A, 0x5C, 80),      // highlight
        nvgRGBA(0x1B, 0x13, 0x20, 255),     // sidebar
        nvgRGBA(0x24, 0x16, 0x20, 255),     // cardBg
        nvgRGBA(0x0F, 0x0A, 0x16, 255),     // dialogBg
        nvgRGB(0xFF, 0xC1, 0x5B),           // teal
        nvgRGB(0xFF, 0x8A, 0x5C),           // status
        nvgRGB(0xD8, 0xC3, 0xBA),           // description
        nvgRGB(0x9B, 0x82, 0x78),           // dimText
        nvgRGBA(0x24, 0x16, 0x20, 255),     // rowBg
        nvgRGBA(0xFF, 0x8A, 0x5C, 90),      // activeRowBg
        nvgRGBA(0x2E, 0x1D, 0x28, 200),     // inactiveRowBg
        nvgRGBA(0xFF, 0x8A, 0x5C, 255),     // trackingBtn
        nvgRGBA(0xFF, 0x8A, 0x5C, 255),     // ctaButton
        nvgRGB(0x37, 0x22, 0x2F),           // sectionHeaderBg
        nvgRGBA(0x0F, 0x0A, 0x16, 255),     // readerBg
        nvgRGBA(0x0F, 0x0A, 0x16, 200),     // errorOverlayBg
        nvgRGBA(0x37, 0x22, 0x2F, 255),     // focusedRowBg
        nvgRGBA(0x0F, 0x0A, 0x16, 255),     // deepBg
        nvgRGBA(0xF6, 0xE9, 0xE3, 100),     // separatorColor
        nvgRGB(0x9E, 0xD3, 0x6A),           // successText
        nvgRGB(0xF6, 0xE9, 0xE3),           // textColor
        nvgRGBA(0x2E, 0x1D, 0x28, 255),     // buttonColor
    };

    // Aurora
    static const ThemePalette auroraPalette = {
        nvgRGB(0x5B, 0xE0, 0xC6),           // accent
        nvgRGB(0x5B, 0xE0, 0xC6),           // secondary
        nvgRGB(0x5B, 0xE0, 0xC6),           // headerText
        nvgRGB(0xBC, 0xD2, 0xCE),           // subtitle
        nvgRGBA(0x5B, 0xE0, 0xC6, 80),      // highlight
        nvgRGBA(0x0D, 0x20, 0x28, 255),     // sidebar
        nvgRGBA(0x10, 0x25, 0x30, 255),     // cardBg
        nvgRGBA(0x08, 0x13, 0x1C, 255),     // dialogBg
        nvgRGB(0x5B, 0xE0, 0xC6),           // teal
        nvgRGB(0x5B, 0xE0, 0xC6),           // status
        nvgRGB(0xBC, 0xD2, 0xCE),           // description
        nvgRGB(0x6F, 0x8A, 0x90),           // dimText
        nvgRGBA(0x10, 0x25, 0x30, 255),     // rowBg
        nvgRGBA(0x5B, 0xE0, 0xC6, 90),      // activeRowBg
        nvgRGBA(0x16, 0x32, 0x3D, 200),     // inactiveRowBg
        nvgRGBA(0x5B, 0xE0, 0xC6, 255),     // trackingBtn
        nvgRGBA(0x5B, 0xE0, 0xC6, 255),     // ctaButton
        nvgRGB(0x1B, 0x3C, 0x48),           // sectionHeaderBg
        nvgRGBA(0x08, 0x13, 0x1C, 255),     // readerBg
        nvgRGBA(0x08, 0x13, 0x1C, 200),     // errorOverlayBg
        nvgRGBA(0x1B, 0x3C, 0x48, 255),     // focusedRowBg
        nvgRGBA(0x08, 0x13, 0x1C, 255),     // deepBg
        nvgRGBA(0xE6, 0xF4, 0xF1, 100),     // separatorColor
        nvgRGB(0x5B, 0xE0, 0xA0),           // successText
        nvgRGB(0xE6, 0xF4, 0xF1),           // textColor
        nvgRGBA(0x16, 0x32, 0x3D, 255),     // buttonColor
    };

    // Synthwave
    static const ThemePalette synthwavePalette = {
        nvgRGB(0xFF, 0x5F, 0xA2),           // accent
        nvgRGB(0x36, 0xE0, 0xE0),           // secondary
        nvgRGB(0xFF, 0x5F, 0xA2),           // headerText
        nvgRGB(0xCD, 0xBC, 0xE0),           // subtitle
        nvgRGBA(0xFF, 0x5F, 0xA2, 80),      // highlight
        nvgRGBA(0x1A, 0x0E, 0x30, 255),     // sidebar
        nvgRGBA(0x1E, 0x11, 0x38, 255),     // cardBg
        nvgRGBA(0x12, 0x07, 0x26, 255),     // dialogBg
        nvgRGB(0x36, 0xE0, 0xE0),           // teal
        nvgRGB(0xFF, 0x5F, 0xA2),           // status
        nvgRGB(0xCD, 0xBC, 0xE0),           // description
        nvgRGB(0x9A, 0x82, 0xC0),           // dimText
        nvgRGBA(0x1E, 0x11, 0x38, 255),     // rowBg
        nvgRGBA(0xFF, 0x5F, 0xA2, 90),      // activeRowBg
        nvgRGBA(0x2A, 0x18, 0x50, 200),     // inactiveRowBg
        nvgRGBA(0xFF, 0x5F, 0xA2, 255),     // trackingBtn
        nvgRGBA(0xFF, 0x5F, 0xA2, 255),     // ctaButton
        nvgRGB(0x32, 0x1C, 0x5C),           // sectionHeaderBg
        nvgRGBA(0x12, 0x07, 0x26, 255),     // readerBg
        nvgRGBA(0x12, 0x07, 0x26, 200),     // errorOverlayBg
        nvgRGBA(0x32, 0x1C, 0x5C, 255),     // focusedRowBg
        nvgRGBA(0x12, 0x07, 0x26, 255),     // deepBg
        nvgRGBA(0xF4, 0xEC, 0xFA, 100),     // separatorColor
        nvgRGB(0x36, 0xE0, 0xE0),           // successText
        nvgRGB(0xF4, 0xEC, 0xFA),           // textColor
        nvgRGBA(0x2A, 0x18, 0x50, 255),     // buttonColor
    };

    // Ocean
    static const ThemePalette oceanPalette = {
        nvgRGB(0x38, 0xBD, 0xF8),           // accent
        nvgRGB(0x38, 0xD0, 0xC8),           // secondary
        nvgRGB(0x38, 0xBD, 0xF8),           // headerText
        nvgRGB(0xBC, 0xD0, 0xDA),           // subtitle
        nvgRGBA(0x38, 0xBD, 0xF8, 80),      // highlight
        nvgRGBA(0x0D, 0x22, 0x30, 255),     // sidebar
        nvgRGBA(0x10, 0x28, 0x36, 255),     // cardBg
        nvgRGBA(0x08, 0x13, 0x1D, 255),     // dialogBg
        nvgRGB(0x38, 0xD0, 0xC8),           // teal
        nvgRGB(0x38, 0xBD, 0xF8),           // status
        nvgRGB(0xBC, 0xD0, 0xDA),           // description
        nvgRGB(0x6F, 0x88, 0x96),           // dimText
        nvgRGBA(0x10, 0x28, 0x36, 255),     // rowBg
        nvgRGBA(0x38, 0xBD, 0xF8, 90),      // activeRowBg
        nvgRGBA(0x16, 0x38, 0x48, 200),     // inactiveRowBg
        nvgRGBA(0x38, 0xBD, 0xF8, 255),     // trackingBtn
        nvgRGBA(0x38, 0xBD, 0xF8, 255),     // ctaButton
        nvgRGB(0x1B, 0x42, 0x54),           // sectionHeaderBg
        nvgRGBA(0x08, 0x13, 0x1D, 255),     // readerBg
        nvgRGBA(0x08, 0x13, 0x1D, 200),     // errorOverlayBg
        nvgRGBA(0x1B, 0x42, 0x54, 255),     // focusedRowBg
        nvgRGBA(0x08, 0x13, 0x1D, 255),     // deepBg
        nvgRGBA(0xE3, 0xEF, 0xF6, 100),     // separatorColor
        nvgRGB(0x4E, 0xCC, 0xA3),           // successText
        nvgRGB(0xE3, 0xEF, 0xF6),           // textColor
        nvgRGBA(0x16, 0x38, 0x48, 255),     // buttonColor
    };

    switch (theme) {
        case AppTheme::TACHIYOMI: return tachiyomiPalette;
        case AppTheme::NEON_VAPORWAVE: return neonVaporwavePalette;
        ...
        case AppTheme::SUNSET: return sunsetPalette;
        case AppTheme::AURORA: return auroraPalette;
        case AppTheme::SYNTHWAVE: return synthwavePalette;
        case AppTheme::OCEAN: return oceanPalette;
...
std::string Application::getThemeString(AppTheme theme) {
    switch (theme) {
        ...
        case AppTheme::SUNSET: return "Sunset";
        case AppTheme::AURORA: return "Aurora";
        case AppTheme::SYNTHWAVE: return "Synthwave";
...
bool Application::loadSettings() {
    ...
    if (static_cast<int>(m_settings.theme) < 0 || static_cast<int>(m_settings.theme) > 16) {
