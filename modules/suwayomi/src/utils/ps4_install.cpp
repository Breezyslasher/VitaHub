/*
    ...
    VitaSuwayomi itself. PS4-Store clears this by uninstalling the title and
...
// instantly (it took VitaSuwayomi down with CE-34878-0 when sceAppInstUtilAppExists
...
int installPkg(const std::string& pkgPath, const std::string& contentName, std::string& err) {
    ...
    // title is the running VitaSuwayomi itself. We deliberately do NOT uninstall
...
int installUpdaterApp(std::string& err) {
    // The helper's pkg ships inside VitaSuwayomi's own pkg (see CMakeLists), so the
    ...
    // VitaSuwayomi, so this install is not blocked by 0x80990088.
    const std::string src = "/app0/updater.pkg";
    const std::string dst = "/data/VitaSuwayomi/updater.pkg";
    ...
    return installPkg(dst, "VitaSuwayomi Updater", err);
}
